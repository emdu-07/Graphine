...
export function MotionGraphs({ motionResult, curveChannel, setCurveChannel, onShowSteps }: MotionGraphsProps) {
  ...
  return (
    <div className="guide-content recording-results">
      <div className="generated-title"><div><p className="kicker">Motion capture complete!</p><h2>Easing Curves</h2></div><WandSparkles size={22} /></div>
      ...
      <div className="sample-summary"><div><span>Time:</span><strong>{motionResult.duration.toFixed(1)}s</strong></div><div><span>KEYFRAMES</span><strong>{motionResult.keyframes.position.length} Position Keyframes<br />{motionResult.keyframes.rotation.length} Rotation Keyframes</strong></div></div>
